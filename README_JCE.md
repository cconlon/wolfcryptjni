...
project default configuration enables neither curve, add `HAVE_ED25519` and
`HAVE_ED448` (with SHAKE256) to its CMake defines to use EdDSA there.
...
- `com.wolfssl.provider.jce.WolfCryptEdDSAParameterSpec(prehash, context)`
  ...
  supplies one (empty when built with null), so for Ed25519 it selects
  Ed25519ctx rather than pure Ed25519.
...
base classes. Keys written with Java serialization on JDK 15+ reference the
overlay classes and cannot be read back on JDK 8-14 or Android, so exchange
keys through `getEncoded()` instead. Java serialization also writes the raw
private key unprotected. Code that reaches the `EdEC` methods by reflection
must resolve them on the `java.security.interfaces` types, since the concrete
overlay classes are not public.
