...
public class WolfSSLKeyStoreTest {
    ...
    private static void createTestObjects()
        ...
        /* Load EdDSA test certs/keys for the curves compiled in. */
        if (FeatureDetect.Ed25519Enabled() &&
            new File(ed25519KeyDer).exists()) {
            try {
                serverKeyEd25519 =
                    derFileToPrivateKey(ed25519KeyDer, "Ed25519");
                serverCertEd25519 = certFileToCertificate(ed25519CertDer);
            }
            catch (Exception e) {
                serverKeyEd25519 = null;
                serverCertEd25519 = null;
            }
        }
    ...
    public void testEdDSAKeyCertMismatchRejected() throws Exception {
        ...
        for (Object[] c : edDsaCurves()) {
            ...
            /* EdDSA private + a freshly generated key's cert-less mismatch:
             * use the other curve's cert when both are available */
            for (Object[] other : edDsaCurves()) {
    ...
    public void testLoadEdDSAWKSFromFile() throws Exception {
        ...
        for (Object[] c : edDsaCurves()) {
            ...
            Assume.assumeTrue("EdDSA WKS file not available " +
                "(run update-jks-wks.sh): " + c[3], f.exists());
            ...
            if (ca.exists()) {
                ...
                assertEquals("EdDSA", caCert.getPublicKey().getAlgorithm());
            }
        }
    }
}
