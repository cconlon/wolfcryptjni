...
public class WolfCryptEdDSAKeyFactoryTest {
    ...
    public static void setUp() {
        ...
        ed25519Enabled = FeatureDetect.Ed25519Enabled();
        ed448Enabled = FeatureDetect.Ed448Enabled();
    ...
    public void pkcs8V2WithPublicKeyIsAccepted() throws Exception {
        ...
        for (String curve : curves()) {
            ...
            assertEquals(0x01, v2[(v2[1] & 0xff) == 0x81 ? 5 : 4]);
    ...
    private static byte[] rfc8410V2(String curve, byte[] priv, byte[] pub) {
        ...
        body[i++] = 0x02; body[i++] = 0x01; body[i++] = 0x01;
        body[i++] = 0x30; body[i++] = 0x05; body[i++] = 0x06;
        body[i++] = 0x03; body[i++] = 0x2b; body[i++] = 0x65;
        body[i++] = oidLast;
        body[i++] = 0x04; body[i++] = (byte)(priv.length + 2);
        body[i++] = 0x04; body[i++] = (byte)priv.length;
        System.arraycopy(priv, 0, body, i, priv.length);
        i += priv.length;
        body[i++] = (byte)0x81; body[i++] = (byte)(pub.length + 1);
        body[i++] = 0x00;
    ...
    public void edecPointOutOfRangeRejected() throws Exception {
        ...
        for (String curve : curves()) {
            ...
            /* y with one bit more than the curve allows, the largest y the
             * range check lets through, and the two smallest values, which
             * decode to small order points native rejects as well */
            BigInteger[] rejected = {
            ...
            for (BigInteger y : rejected) {
                ...
                try {
                    ...
                    fail("y = " + y.bitLength() + " bits accepted for " +
                        curve);
                } catch (InvalidKeySpecException e) {
                    /* expected */
                }
    ...
    public void malformedDerRejected() throws Exception {
        ...
        byte[][] bad = { new byte[0], new byte[] { 0x30, 0x00 },
        ...
        for (String curve : curves()) {
            ...
            try {
                kf.generatePublic(edecPublicKeySpec(other, Arrays.copyOf(
                    rawPub, other.equals("Ed448") ? 57 : 32)));
