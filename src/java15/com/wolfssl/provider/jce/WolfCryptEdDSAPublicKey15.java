 ...
 * is inherited from WolfCryptEdDSAPublicKey.
 */
final class WolfCryptEdDSAPublicKey15 extends WolfCryptEdDSAPublicKey
    ...
    private Object writeReplace() {
        WolfCryptEdDSAPublicKey base;

        if (isDestroyed()) {
            base = new WolfCryptEdDSAPublicKey(curve(),
                new byte[curve().getPublicKeySize()], new byte[0]);
            base.destroy();
            return base;
        }

        return new WolfCryptEdDSAPublicKey(curve(), getRawPublicKey(),
            getEncoded());
    }
