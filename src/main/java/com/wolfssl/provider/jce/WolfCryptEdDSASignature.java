 ...
 *
 * <p>EdDSA signs the whole message (twice, internally), so this
 * implementation buffers the message in a ByteArrayOutputStream during
 * engineUpdate and consumes it on engineSign / engineVerify.</p>
 *
 ...
 * built with null), so for Ed25519 it always selects Ed25519ctx. Use
 * WolfCryptEdDSAParameterSpec(false) for pure Ed25519.</p>
 *
...
public abstract class WolfCryptEdDSASignature extends SignatureSpi {
    ...
    /**
     * EdDSA has no encoded algorithm parameters (RFC 8410), so this
     * always returns null.
     *
     * @return null
     */
