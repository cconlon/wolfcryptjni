...
public class WolfCryptPKIXCertPathBuilder extends CertPathBuilderSpi {
    ...
    private X509Certificate findTargetCertificate(PKIXBuilderParameters params)
        ...
        X509Certificate targetCert = x509Selector.getCertificate();
        if (targetCert != null) {
    ...
    /**
     * Check if a certificate is issued by a trust anchor.
     *
    ...
    private List<X509Certificate> findIssuers(X509Certificate cert,
         ...
         * Trust anchors should also be CA certificates (basicConstraints >= 0)
         * to be valid issuers in a certificate chain. */
        for (TrustAnchor anchor : anchors) {
            X509Certificate anchorCert = anchor.getTrustedCert();
            if (anchorCert != null &&
    ...
    /**
     * Build the certificate path from target to trust anchor.
     *
    ...
    /**
     * Find the trust anchor that issued the last certificate in the path.
     *
     ...
     *
     * Uses wolfSSL_X509_verify_cert() that does certificate path building
     * with automatic backtracking when a candidate issuer fails verification.
     *
     * @param targetCert the target (end entity) certificate
     * @param params the PKIX builder parameters
     *
    ...
    private NativeChainResult buildAndVerifyPathNative(
        ...
        try {
            ...
            for (TrustAnchor anchor : anchors) {
                ...
                X509Certificate anchorCert = anchor.getTrustedCert();
                if (anchorCert != null) {
                    byte[] anchorDer = anchorCert.getEncoded();
            ...
            if (certStores != null) {
                for (CertStore store : certStores) {
                    try {
                        ...
                        log("error collecting intermediates: " +
                            e.getMessage());
                    }
                }
            }
            ...
            byte[] targetDer = targetCert.getEncoded();
            ...
            if (foundAnchor == null) {
                ...
                if (foundAnchor != null) {
                     ...
                     * (self-signed) but with different encoding. Can happen
                     * when wolfSSL re-encodes certificates. */
                    if (isSelfSigned(lastCert) &&
                        lastCert.getSubjectX500Principal().equals(
                            foundAnchor.getTrustedCert()
                        ...
                        log("path ends at certificate issued by " +
                            "trust anchor: " + foundAnchor.getTrustedCert()
            ...
            log("path built successfully, trust anchor: " +
                foundAnchor.getTrustedCert().getSubjectX500Principal()
                    .getName());
            ...
            throw new CertPathBuilderException(
                "Failed to process certificates", e);
    ...
    public CertPathBuilderResult engineBuild(CertPathParameters params)
        ...
        NativeChainResult result = buildAndVerifyPathNative(targetCert,
            pkixParams);
        path = result.path;
