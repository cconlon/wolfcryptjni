...
public class WolfCryptEdDSAPrivateKey implements PrivateKey, Destroyable {
    ...
    public WolfCryptEdDSAPrivateKey(byte[] pkcs8Der)
        ...
        if (found != null) {
             ...
             * we can't parse). Try each compiled-in curve's native decoder in
             * turn. */
            ...
            if (found == null) {
                if (last == null) {
                    throw new IllegalArgumentException(
                        "Ed25519 and Ed448 are not compiled into native " +
                        "wolfSSL");
                }
                throw new IllegalArgumentException(
                    "Not a valid Ed25519 or Ed448 PKCS#8 key", last);
            }
        }
        this.rawPriv = r[0];
        this.rawPub = r[1];
    ...
    /**
     * Hash code based on the curve and the (derived) public key.
     *
    ...
    private Object readResolve() {
        synchronized (stateLock) {
            ...
            return WolfCryptEdDSAKeys.trustedPrivateKey(curve, rawPriv, rawPub,
                encoded);
        }
    }
