...
public class Ed25519 extends NativeStruct {
    ...
    /* Native sign / verify function selectors, must match JNI enums */
     ...
     * @throws IllegalArgumentException if size is not {@link #ED25519_KEY_SIZE}
     ...
     *         when no key has been loaded or the object was released)
     */
     ...
     *
     * <p>Key pair is validated on import, guards against signing with a
     * mismatched public key.</p>
     *
    ...
    public byte[] exportPublic()
        ...
        synchronized (stateLock) {
            throwIfKeyNotLoaded();
            synchronized (pointerLock) {
                /* older native releases export zeros instead of failing */
                if (!ed25519_key_pubKeySet()) {
                    throw new WolfCryptException(
     ...
     *         released.
     *
     * @deprecated use {@link #sign(byte[])}. A null message now throws
     *             IllegalArgumentException instead of WolfCryptException.
     */
    @Deprecated
        ...
        synchronized (stateLock) {
            ...
            synchronized (pointerLock) {
                ...
                try {
                    ...
                    /* BAD_FUNC_ARG here is a malformed signature (S not
                     * below the group order) or a public key that is not
                     * on the curve, both "does not verify" */
     ...
     *             Null arguments now throw IllegalArgumentException and a
     *             wrong-length signature returns false instead of throwing.
     */
    @Deprecated
     ...
     * is not validated on the streaming path, a wrong-sized input fails to
     ...
     * started stream stays active until verifyFinal() or another
     * verifyInit(), so finish or restart it before other operations.
     *
        ...
        synchronized (stateLock) {
            ...
            synchronized (pointerLock) {
                ...
                try {
                    ...
                    /* Malformed signature treated as "does not verify" */
                    if (e.getError() != WolfCryptError.BAD_FUNC_ARG) {
                        throw e;
                    }
    ...
    /**
     * Feed a message segment to a streaming verify.
     *
     ...
     *         not been called or no key is loaded, or object has been released
     */
     ...
     * immediately usable for signing.
     *
     ...
     * produced, deriving the public key first if it is not loaded. Note that
     * some decoders reject the v2 form.
     *
     ...
     * @throws WolfCryptException if native operation fails or no private
     *         key is loaded
     * @throws IllegalStateException if key has not been set, if a
     *         streaming verify is in progress, if object fails to
