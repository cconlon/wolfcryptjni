 ...
 * context is longer than 255 bytes (native contextLen is a byte). NULL
...
#if defined(HAVE_ED25519_KEY_EXPORT) && !defined(NO_ASN)
  ...
  (JNIEnv* env, jobject this)
{
    ...
    PRIVATE_KEY_UNLOCK();
    ret = wc_ed25519_make_public(key, key->p, (word32)sizeof(key->p));
    PRIVATE_KEY_LOCK();

    if (ret == 0) {
        /* Mirror into key->k as wc_ed25519_make_key() does, the
         * private||public exports copy key->k. Relies on key->k being the
         * fixed ED25519_PRV_KEY_SIZE array declared in ed25519.h */
        XMEMCPY(key->k + ED25519_KEY_SIZE, key->p, ED25519_PUB_KEY_SIZE);
        result = ed25519_jni_new_byte_array(env, key->p,
            (word32)sizeof(key->p));
    }
    else {
        if (!(*env)->ExceptionCheck(env)) {
  ...
  (JNIEnv* env, jobject this)
{
    ...
    if (ret == 0) {
        ...
        if (pubArr != NULL) {
            ...
            /* Alloc failed, OutOfMemoryError pending */
            result = NULL;
        }
    }
    else {
...
{
#if defined(HAVE_ED25519_KEY_IMPORT) && !defined(NO_ASN)
    int ret = 0;
    ed25519_key* key = NULL;
    byte* der = NULL;
    word32 derSz = 0;
