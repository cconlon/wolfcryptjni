    ...
    int errorCode)
{
    ...
    snprintf(msgBuf, sizeof(msgBuf), "%s: %s (error %d)",
        prefix, errStr, errorCode);
   ...
   jobjectArray intermediateCertsDer, jint maxPathLength)
{
    ...
    for (i = 0; i < chainLen; i++) {
        int derLen = 0;
        WOLFSSL_X509* cert = wolfSSL_sk_X509_value(chain, i);
        ...
        (*env)->SetByteArrayRegion(env, derArray, 0, derLen,
            (const jbyte*)der);
