 ...
 * (both are included in --enable-all). If a
 * curve is not compiled into native wolfCrypt it is skipped.
 */
public class EdDSAExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String curve : CURVES) {
            /* Detect whether the curve is available in this wolfJCE build.
             * If native wolfSSL was built without it, the KeyPairGenerator
             * lookup fails. */
            try {
                KeyPairGenerator.getInstance(curve, "wolfJCE");
            } catch (Exception e) {
                System.out.println("\n[" + curve + "] not available in " +
                    "this wolfJCE build, rebuild native wolfSSL with " +
                    "--enable-" + curve.toLowerCase() + " (or --enable-all)");
                continue;
            }
            runCurve(curve, msg);
            ran++;
